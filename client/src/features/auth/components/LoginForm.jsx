...
function LoginForm({ onShowRegister }) {
  ...
  useEffect(() => {
    ...
    const onLoad = () => {
      ...
      googleCodeClientRef.current = window.google.accounts.oauth2.initCodeClient({
        ...
        callback: (response) => {
          if (response.error) {
            setErrors({ general: 'Không thể đăng nhập bằng Google. Vui lòng thử lại.' });
            return;
          }
  ...
  const handleGoogleLogin = () => {
    ...
    if (!googleCodeClientRef.current) {
      setErrors({ general: 'Dịch vụ đăng nhập Google chưa sẵn sàng. Vui lòng tải lại trang.' });
      return;
    }
    // Must be called synchronously from this click so browsers allow the OAuth popup.
      ...
      <div className="mt-4 sm:mt-8 w-full">
        ...
        <div className="w-full">
          {googleClientId ? (
            ...
            <p className="text-center text-xs text-amber-600 dark:text-amber-300">Đăng nhập Google chưa được cấu hình.</p>
          )}
        </div>
      </div>
      </div>
