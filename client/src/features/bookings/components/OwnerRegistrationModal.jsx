...
export default function OwnerRegistrationModal({ isOpen, onClose, onAgree }) {
  ...
  const agree = async () => {
    ...
    setError('');
    try { await onAgree(); }
    catch (err) { setError(err.message || 'Không hoàn tất được đăng ký'); }
    ...
    <div className="fixed inset-0 z-[1060] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="owner-water-card relative flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-cyan-200/50 bg-white shadow-2xl dark:bg-[#071c2d]" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5 text-sm text-slate-700 dark:text-slate-200 sm:p-6">
          <div className="rounded-2xl border border-cyan-200 bg-cyan-50/80 p-4 dark:border-cyan-400/20 dark:bg-cyan-400/10"><p className="font-black text-cyan-800 dark:text-cyan-200">Mức phí dự kiến: 150.000đ / sân / tháng</p><p className="mt-1 text-xs text-cyan-700/80 dark:text-cyan-100/80">Hiện tại SportGo ghi nhận đăng ký; chưa tích hợp thu phí tự động.</p></div>
          ...
          {error && <p role="alert" className="rounded-xl bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 dark:bg-red-950/30 dark:text-red-300">{error}</p>}
        ...
        <div className="flex shrink-0 flex-col-reverse gap-2 border-t border-slate-100 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5 sm:flex-row sm:justify-end sm:gap-3 sm:p-5"><button type="button" onClick={onClose} disabled={isSubmitting} className="rounded-xl px-5 py-2.5 text-sm font-bold text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10">Để sau</button><button type="button" onClick={agree} disabled={isSubmitting} className="rounded-xl bg-gradient-to-r from-[#087f8c] to-[#0aa4b4] px-5 py-2.5 text-sm font-black text-white shadow-lg shadow-cyan-700/20 transition hover:-translate-y-0.5 disabled:opacity-50">{isSubmitting ? 'Đang đăng ký…' : 'Tôi đồng ý & tiếp tục'}</button></div>
      </div>
    </div>
  );
}
