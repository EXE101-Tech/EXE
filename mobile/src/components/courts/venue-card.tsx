    ...
    <Pressable onPress={onPress}>
      <Card className="overflow-hidden">
        <View className="h-40 w-full bg-slate-100 dark:bg-slate-800">
          ...
          {venue.rating != null ? (
            ...
            <View className="absolute right-3 top-3 rounded-full bg-black/50 px-2.5 py-1">
              <Text className="text-xs font-bold text-white">Chưa đánh giá</Text>
            </View>
          )}
        ...
        <View className="gap-3 p-4">
          <View>
            <Text className="text-base font-black text-brand dark:text-brand-dark" numberOfLines={2}>
              {venue.name}
            </Text>
          ...
          <View className="flex-row items-center justify-between border-t border-border pt-2.5 dark:border-border-dark">
            <Text className="text-xs font-semibold text-slate-600 dark:text-slate-300">Giá thuê khung giờ (30p):</Text>
          ...
          <View>
            <Text className="mb-1.5 text-[11px] font-extrabold uppercase tracking-wide text-slate-500 dark:text-slate-400">
              Tiện ích & dịch vụ
            </Text>
            {facilities.length > 0 ? (
              ...
              <Text className="text-xs italic text-slate-400 dark:text-slate-500">Chưa cập nhật tiện ích</Text>
            )}
          </View>
          ...
          <View className="border-t border-border pt-3 dark:border-border-dark">
            {isOwnedByUser ? (
              ...
              <TouchableOpacity onPress={onPress} className="overflow-hidden rounded-xl">
                <LinearGradient
                  colors={ctaGradient}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 1 }}
